...
class Main 
{
	...
	//가로세로 N,M 최대 500
	static int N,M;
	//방왔던 길을 되돌아가면 안되기때문에 visited가 빠지면 안됨★ 
	...
	public static void main(String[] args) 
	{
		...
		N= scan.nextInt(); //int를 또 선언하지 않도록 조심
		M= scan.nextInt();
		for(int i=0; i<N; i++){
			for(int j=0; j<M; j++){
		...
		for(int i=0; i<N; i++){
			for(int j=0; j<M; j++){
			//깊이탐색 ㄱ,ㄴ 형태찾기
			DFS(0,i,j,0); //4번째 인자는 sum인데, 정점이 움직일 때무다 sum=0으로 자동 초기화된다. 
			//DFS로 안되는 ㅗ,ㅓ,ㅏ,ㅜ 예외찾기
			solve(i,j);
			}
		}
	...
	static void DFS(int depth, int row, int col, int sum){
		...
		//4번 탐색하고 멈춰야함
		if(depth==limit-1){ //3보다는 limit-1이 유지보수 용이
